...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      setUser(currentUser);
      if (currentUser) {
        setInspectorName(currentUser.displayName || currentUser.email.split("@")[0].toUpperCase());
        try {
          const userDocRef = doc(db, "users", currentUser.uid);
          const userSnap = await getDoc(userDocRef);
          if (userSnap.exists() && userSnap.data().role === "admin") {
            setIsAdmin(true);
          } else if (currentUser.email.toLowerCase().includes("admin") || currentUser.email.toLowerCase().includes("terekked")) {
            setIsAdmin(true);
          } else {
            setIsAdmin(false);
          }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setLoginError("Failed to sign in with Google. Please check your browser popup settings.");
    }
  };
        ...
        <div className="max-w-md w-full bg-white rounded-3xl p-8 shadow-2xl border border-slate-800 space-y-6">
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full py-3.5 px-4 bg-white border border-slate-300 hover:bg-slate-50 rounded-xl font-bold text-slate-700 shadow-sm flex items-center justify-center gap-3 transition-all hover:shadow"
            >
              <img 
          ...
          <p className="text-[11px] text-center text-slate-400">
            Sign in using your authorized management Google account.
          </p>
        </div>
      </div>
    );
